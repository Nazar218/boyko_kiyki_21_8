...
class Student {
  ...
  Student copyWith(firstName, lastName, department, gender, grade) {
  ...
  static Department departmentFromJson(String departmentString) {
    return Department.values.firstWhere(
      (d) => d.toString().split('.').last == departmentString,
      orElse: () => throw ArgumentError('Invalid: $departmentString'),
    );
  }


  static String departmentToJson(Department department) {
    ...
    for (final item in data.entries) {
      loadedItems.add(
        Student(
          ...
          department: departmentFromJson(item.value['department']!),
      ...
      body: json.encode(
        {
          ...
          'last_name': lastName,
          'department': departmentToJson(department),
